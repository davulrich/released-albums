...
var API_ID = "d4fc36d48695441abadd88853e8a6829";
var API_SECRET = "acc385dacf8d4f798dd4f16825951603";
...
var REDIRECT_URI = "http://127.0.0.1:8080";
...
async function showSettings() {
  ...
  elementSettings.append(
    `<div class="settings-section" id="settings-notifications"><h3>Notifications</h3><p>Enable or disable browser notifications</p><ul class="playlists settings-playlist"> ` +
      notificationsLi +
      `</ul></div>`
  );

  elementSettings.append(
    `<div class="settings-section" id="settings-device"><h3>Default device</h3><p>Set your default device to play releases.</p></div>`
  );
  var elementSettingsDevice = $("#settings-device");
  if (user.product != "premium") {
    elementSettingsDevice.append(
      `<p><i class="fas fa-times"></i> This feature is available only for Spotify Premium users.</p>`
    );
  } else if (devices.length < 1) {
    elementSettingsDevice.append(
      `<p><i class="fas fa-times"></i> No device found</p>`
    );
  } else {
  ...
  elementSettings.append(
    `<div class="settings-section" id="settings-variousart"><h3>Various artists</h3><p>Show or hide releases from Various artists.</p><ul class="playlists settings-variousart-showhide"> ` +
      varArtLi +
      `</ul></div>`
  );
  ...
  elementSettings.append(
    `<div class="settings-section" id="settings-playlist-position"><h3>Position of added release to the playlist</h3><p>Set the position (first or last) of added releases to the playlist.</p><ul class="playlists settings-playlist-position"> ` +
      plPositionLi +
      `</ul></div>`
  );

  elementSettings.append(
    `<div class="settings-section" id="settings-playlist"><h3>Default playlist</h3><p>Set your default playlist to quickly add releases.</p></div>`
  );
  ...
  if (libraryPlaylists.length < 1) {
    ...
    elementSettings.append(
      `<div class="settings-section" id="settings-youtubemusic"><h3>YouTube Music Search</h3><p>Show or hide YouTube Music search icon.</p><ul class="playlists settings-youtubemusic"> ` +
        youtubeLi +
        `</ul></div>`
    );
  }
...
$(document).on("click", ".notifications-enable", async function (e) {
  ...
  if (!window.Notification) {
    ...
    if (Notification.permission === "granted") {
      ...
      elementNotifications.prop(
        "title",
        "Click to disable browser notifications"
      );
      elementNotifications.html(
...
$(document).on("click", ".playlist-default-set", async function (e) {
  ...
  if (defaultPlaylist) {
    if (playlistId != defaultPlaylist.id) {
      ...
      // titulek
      playlistDefaultDiv.prop(
        "title",
        `Set playlist '` + playlistDiv.text() + `' as default`
      );
    }
  }
  // ikona
...
$(document).on("click", ".album-playlist-add-default", async function (e) {
  ...
  if (params.show == "podcasts") {
    ...
    inPlaylist = await libraryIsSongInPlaylist(
...
$(document).on("click", ".settings .youtubemusic", async function (e) {
  ...
  if (youtubeMusic === true) {
    ...
    elementYtMusic.html(
      `<i class="fas fa-times"></i>YouTube Music search disabled`
    );
    youtubeMusic = false;
  } else {
    // enable
    ...
    elementYtMusic.html(
      `<i class="fas fa-check"></i>YouTube Music search enabled`
    );
    youtubeMusic = true;
  }
});
...
$(document).on("click", ".album-playlist-add-new", async function (e) {
  ...
  // vytvoreni playlistu
  var newPlaylist = await createPlaylist(
    release.artistsString + " - " + release.name
  );
  if (newPlaylist === null) {
    return;
  }
...
$(document).on("click", ".device-default-set", async function (e) {
  ...
  if (defaultDevice) {
    if (deviceId != defaultDevice.id) {
      ...
      // titulek
      deviceDefaultDiv.prop(
        "title",
        `Set device '` + deviceDiv.text() + `' as default`
      );
    }
  }
  // ikona
...
$(document).on("click", ".playlist-position-last", async function (e) {
  ...
  elementPlaylistSettings.html(
    `<i class="fas fa-angle-down"></i></i>Last position`
  );
...
$(document).on("click", ".settings .variousart", async function (e) {
  ...
  if (variousArtists === true) {
    ...
    elementVarArt.html(
      `<i class="fas fa-times"></i>Releases from Various artists are hidden`
    );
    variousArtists = false;
  } else {
    // enable
    ...
    elementVarArt.html(
      `<i class="fas fa-check"></i>Releases from Various artists are displayed`
    );
    variousArtists = true;
  }
});
